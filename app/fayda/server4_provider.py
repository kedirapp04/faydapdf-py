...
class Server4Provider(FaydaProvider):
    ...
    async def verify_pdf(self, session, otp: str) -> dict:
        ...
        try:
            ...
            # Render the ID PDF. PDF_ENGINE='js' uses the bundled faydapdf-railway
            # pdfGenerator via a Node subprocess (byte-for-byte the JS output; a separate
            # process, so it never blocks the loop). Falls back to the Python renderer
            # (in a thread) if Node/bundle is missing or the subprocess fails.
            from . import js_render
            pdf_bytes, name = await js_render.render(user, engine=config.PDF_ENGINE)
